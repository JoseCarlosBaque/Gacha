 ...
 * @author joseb
 */
public class BannerInicial extends javax.swing.JFrame {

    /**
     * Creates new form BannerInicial
     */
    public BannerInicial() {
        initComponents();
        this.setLocationRelativeTo(null);
        ImageIcon imagen = new ImageIcon("src\\main\\java\\images\\Banner1.png");
    ...
    private void initComponents() {
        ...
        jShowPersonajes = new javax.swing.JButton();
        ...
        jShowPersonajes.setText("Personajes");
        jShowPersonajes.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jShowPersonajesActionPerformed(evt);
            }
        });
        jPanel1.add(jShowPersonajes, new org.netbeans.lib.awtextra.AbsoluteConstraints(400, 10, -1, -1));
    ...
    private void jShowPersonajesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jShowPersonajesActionPerformed
        for (int i = 0; i < banner_content.size(); i++) {
            JOptionPane.showMessageDialog(null, banner_content.get(i).getNombre());
        }
    }//GEN-LAST:event_jShowPersonajesActionPerformed
    ...
    private void jSingleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jSingleActionPerformed
        if (user.getItems().size() > 0) {
            if (user.getItems().get(0).getCantidad() >= 1) {
                int x = (int) (Math.random() * banner_content.size());
                if (banner_content.get(x).isConseguido() == false) {
                    try {
                        Personaje pj = (Personaje) banner_content.get(x).clone();
                        banner_content.get(x).setConseguido(true);
                        pj.setId_personaje(cont++);
                        user.getEquipo().add(pj);
                        JOptionPane.showMessageDialog(null, "Has conseguido a " + banner_content.get(x).getNombre());
                    } catch (CloneNotSupportedException ex) {
                        JOptionPane.showMessageDialog(null, "Error: " + ex.toString());
                    }
                } else {
                    for (int i = 0; i < user.getEquipo().size(); i++) {
                        if (banner_content.get(x).getNombre().equals(user.getEquipo().get(i).getNombre())) {
                            if (banner_content.get(x).getTipo().equals(user.getEquipo().get(i).getTipo())) {
                            ...
                            } else if (banner_content.get(x).getTipo().equals(user.getEquipo().get(i).getTipo())) {
                ...
                user.getItems().get(0).setCantidad(user.getItems().get(0).getCantidad() - 1);
                jLabel2.setText("Te quedan: " + user.getItems().get(0).getCantidad() + user.getItems().get(0).getNombre());
            } else {
                JOptionPane.showMessageDialog(null, "No tienes suficiente " + items.get(0).getTipo() + " Necesitas 1 o mas para tirar");
            }
        } else {
            JOptionPane.showMessageDialog(null, "No tienes suficiente " + items.get(0).getTipo() + " Necesitas 1 o mas para tirar");
        }
    ...
    private void jMultiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMultiActionPerformed
        if (cont_free == 0) {
            ...
            if (user.getItems().get(0).getCantidad() >= 50) {
                for (int j = 0; j < 10; j++) {
                    int x = (int) (Math.random() * banner_content.size());
                    if (banner_content.get(x).isConseguido() == false) {
                        try {
                ...
                user.getItems().get(0).setCantidad(user.getItems().get(0).getCantidad() - 50);
                jLabel2.setText("Te quedan: " + user.getItems().get(0).getCantidad() + user.getItems().get(0).getNombre());
            } else {
                JOptionPane.showMessageDialog(null, "No tienes suficiente " + items.get(0).getTipo() + " Necesitas 50 o mas para tirar");
            }
        } else {
            JOptionPane.showMessageDialog(null, "No tienes suficiente " + items.get(0).getTipo() + " Necesitas 50 o mas para tirar");
        }
    ...
    private void jRatesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jRatesActionPerformed
        JOptionPane.showMessageDialog(null, "Probabilidad Nuevo Personaje: " + banners.get(0).getProb_nuevo() + "%\nProbabilidad de Sparking: " + banners.get(0).getProb_sp()
                + "%\nProbabilidad de Extreme: " + banners.get(0).getProb_ex() + "%\nProbabilidad de Hero: " + banners.get(0).getProb_hero() + "%");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(BannerInicial.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new BannerInicial().setVisible(true);
            }
        });
    }
    ...
    private javax.swing.JButton jShowPersonajes;
